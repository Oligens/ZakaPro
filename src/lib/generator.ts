...
const methodsLit = (methods: Source[]): string => {
  if (methods.length > 0) {
    return "[" + methods.map(function (method) { return JSON.stringify(method); }).join(", ") + "]";
  }
  return "[\"moncash\", \"natcash\"]";
};

export function sdkSnippet(app: ZakaApp, o: SnippetOpts): string {
  const webhook = o.webhook || DEFAULT_WEBHOOK;
  const apiBase = o.apiBase || DEFAULT_API_BASE;
  ...
  return [
    ...
    "const ZAKAPRO_API_BASE = " + js(apiBase) + ";",
    ...
    "    throw new Error(body.error || \"Impossible de charger les plans.\");",
    "  }",
    ...
    "    option.textContent = plan.name + \" — \" + Number(plan.amount).toLocaleString() + \" HTG\";",
    ...
    "    try {",
    "      const plans = await loadZakaPlans();",
    "      const planSelect = document.querySelector(\"#zaka-plan\");",
    "      const selectedId = (planSelect && planSelect.value) || (plans[0] && plans[0].id);",
    "      const plan = plans.find(function (item) { return item.id === selectedId; });",
    "      if (!plan) {",
    "        throw new Error(\"Aucun plan valide n\\'est disponible.\");",
    "      }",
    ...
    "        customer: { email: typeof clientEmail === \"string\" ? clientEmail : \"\" },",
    "        meta: { planId: plan.id, appId: plan.appId },",
    "        onSuccess: function (tx) { console.log(\"Paiement confirmé\", tx.reference, plan.id); },",
    "        onCancel: function () { console.log(\"Paiement annulé\"); },",
    "      });",
    "    } catch (error) {",
...
export function hubButtonSnippet(app: ZakaApp, plan: ZakaPlan): string {
  ...
  return [
    "<!-- Bouton Hub ZakaPro — planId -->",
    "<a href=\"" + link + "\" target=\"_blank\" rel=\"noopener\" style=\"display:inline-block;background:#EAB308;color:#090D16;border-radius:10px;padding:14px 26px;font-weight:800;font-family:sans-serif;text-decoration:none\">",
    "  Peye kounye a — " + fmtNum(plan.amount) + " HTG",
    "</a>",
    "<!-- Le Hub utilise planId : aucun montant n'est codé en dur côté validation. -->"
  ].join("\n");
}

export function generateCurlSnippet(
  app: { publicKey: string },
  o: { planId?: string; amount?: number }
): string {
  const planId = o.planId || "PLAN_ID_SELECTIONNE";
  const amountPart = o.amount !== undefined
    ? ",\n    \"amount\": " + String(Number(o.amount))
    : "";
  const shellQuote = String.fromCharCode(39);

  return "curl -X POST https://zakapro.vercel.app/api/checkout/intent \\\n" +
...
export function curlSnippet(app: ZakaApp, o: SnippetOpts): string {
  return generateCurlSnippet(app, {
    planId: o.planId,
    amount: o.amount
  });
}
