...
export function AppIntegrationDedicated() {
  ...
  useEffect(() => {
    ...
    setAmount(appPlans[0]?.amount ?? 1500);
    setHubPlanIdx(0);
        ...
        <div className="rounded-xl border border-edge bg-panel p-4 shadow-card sm:p-5">
          ...
          <div className="grid gap-3 lg:grid-cols-2">
            <div className="rounded-lg border border-edge bg-abyss/70 p-3.5">
              <div className="flex items-center justify-between"><p className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-fog2">Clé publique (app_key)</p><span className="inline-flex items-center gap-1 rounded bg-mint/12 px-1.5 py-0.5 text-[9px] font-extrabold uppercase tracking-wider text-mint"><IconCheck width={9} height={9} strokeWidth={3.5} /> Exposable côté client</span></div>
              <div className="mt-2 flex items-center gap-2"><code className="min-w-0 flex-1 truncate font-mono text-[12px] text-gold">{app.publicKey}</code><CopyBtn text={app.publicKey} label="" className="px-2 py-1" /><button type="button" onClick={() => zaka.regenerateAppKey(app.id, "public")} className="grid h-8 w-8 shrink-0 cursor-pointer place-items-center rounded-lg border border-edge2 bg-panel2 text-fog transition-colors hover:border-gold/50 hover:text-gold" aria-label="Régénérer la clé publique"><IconRefresh width={13} height={13} /></button></div>
            </div>
            <div className="rounded-lg border border-edge bg-abyss/70 p-3.5">
              <div className="flex items-center justify-between"><p className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-fog2">Clé secrète (webhooks)</p><span className="rounded px-1.5 py-0.5 text-[9px] font-extrabold uppercase tracking-wider" style={{ color: "#EC4899", background: "#EC489918" }}>Serveur uniquement</span></div>
              <div className="mt-2 flex items-center gap-2"><code className="min-w-0 flex-1 truncate font-mono text-[12px] text-nat">{showSecret ? app.secretKey : app.secretKey.slice(0, 10) + "•••••••••••••••"}</code><button type="button" onClick={() => setShowSecret((v) => !v)} className={`grid h-8 w-8 shrink-0 cursor-pointer place-items-center rounded-lg border border-edge2 bg-panel2 transition-colors ${showSecret ? "text-gold" : "text-fog hover:text-snow"}`} aria-label="Afficher la clé secrète"><IconEye width={13} height={13} /></button><CopyBtn text={app.secretKey} label="" className="px-2 py-1" /><button type="button" onClick={() => zaka.regenerateAppKey(app.id, "secret")} className="grid h-8 w-8 shrink-0 cursor-pointer place-items-center rounded-lg border border-edge2 bg-panel2 text-fog transition-colors hover:border-gold/50 hover:text-gold" aria-label="Régénérer la clé secrète"><IconRefresh width={13} height={13} /></button></div>
            </div>
          </div>
        </div>
      </Reveal>
      ...
      <div className="grid gap-4 lg:grid-cols-5">
        ...
        <Reveal delay={80} className="lg:col-span-3">
          <div className="flex h-full min-h-[420px] flex-col rounded-xl border border-edge bg-panel shadow-card">
            <div className="flex items-center gap-1 overflow-x-auto border-b border-edge px-3 py-2.5">{SNIPPET_TABS.map((t) => <button key={t.key} type="button" onClick={() => setTab(t.key)} className={`shrink-0 cursor-pointer rounded-lg px-3 py-1.5 font-mono text-[11px] font-bold transition-all ${tab === t.key ? "bg-gold/14 text-gold" : "text-fog hover:bg-panel2 hover:text-snow"}`}>{t.label}</button>)}<div className="ml-auto flex shrink-0 items-center gap-2 pl-2"><CopyBtn text={snippets[tab]} label="Copier le code" /></div></div>
