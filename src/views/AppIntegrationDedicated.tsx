...
import { curlSnippet, hubButtonSnippet, listenerSnippet, multiPlanButtonsSnippet, sdkSnippet, webhookSnippet } from "../lib/generator";
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-4 lg:grid-cols-5">
      <Reveal className="lg:col-span-2"><div className="space-y-4">
        <div className="rounded-xl border border-edge bg-panel p-4 shadow-card sm:p-5"><SectionHead title="Plans de l'application" sub="Chaque plan devient un bouton de paiement indépendant"/>
          {appPlans.length === 0 ? <p className="rounded-lg border border-edge bg-panel2 p-3 text-xs text-fog2">Aucun plan. Créez vos plans dans « Plans & Hub ».</p> : <div className="space-y-3"><div><label className={labelCls} htmlFor="int-plan">Plan de référence pour le bouton/cURL</label><select id="int-plan" value={selectedPlan?.id ?? ""} onChange={e=>setSelectedPlanId(e.target.value)} className={inputCls+" cursor-pointer"}>{appPlans.map(p=><option key={p.id} value={p.id} className="bg-panel text-snow">{p.name} — {fmtNum(p.amount)} HTG{p.delivery?" (+ frais zone)":""}</option>)}</select></div><div className="rounded-lg border border-gold/20 bg-gold/5 p-3 text-[11px] text-fog2"><strong className="text-gold">{appPlans.length} plan(s)</strong> détecté(s). Le SDK Web utilise l'<code className="text-fog">app_key</code>, récupère <code className="text-fog">plans[]</code>, puis crée un bouton indépendant avec son <code className="text-fog">planId</code>.</div></div>}
