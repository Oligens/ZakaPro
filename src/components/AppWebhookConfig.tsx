...
export function AppWebhookConfig({ app }: { app: { id: string; publicKey: string; webhookUrl?: string } }) {
  ...
  useEffect(() => { setUrl(app.webhookUrl || ""); setStatus(null); }, [app.id, app.webhookUrl]);
  ...
  const save = async () => {
    ...
    try {
      await api.updateAppWebhook(app.id, value);
    ...
    <section className="relative overflow-hidden rounded-xl border p-4 shadow-card sm:p-5" style={{ borderColor: "#EAB30866", background: "#151106" }}>
      ...
      <div className="relative">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1"><h2 className="text-sm font-bold text-snow">Webhook dédié à cette application</h2><p className="mt-1 text-[11px] leading-relaxed text-fog">Cette URL est isolée par application. Elle est prioritaire sur le webhook global configuré dans Paramètres.</p></div>
        </div>
        <div className="mt-4 grid gap-2 sm:grid-cols-[1fr_auto]">
          ...
          <button type="button" disabled={saving} onClick={() => void save()} className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-lg bg-gold px-4 py-2.5 text-xs font-extrabold text-ink shadow-glow transition-all hover:bg-goldsoft disabled:cursor-wait disabled:opacity-60">{saving ? <IconRefresh width={14} height={14} className="animate-spin" /> : <IconCheck width={14} height={14} strokeWidth={2.8} />}{saving ? "Enregistrement…" : "Enregistrer"}</button>
        </div>
