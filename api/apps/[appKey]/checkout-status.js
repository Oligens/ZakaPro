...
export default async function handler(req,res){
  ...
  try{
    ...
    if(!appKey||!reference) return sendJson(res,400,{error:"appKey et reference requis.",code:"missing_parameters"});

    const appResult=await pool.query(
      `SELECT id, public_key FROM apps WHERE id::text=$1 OR public_key=$1 LIMIT 1`,
      [appKey]
    );
    ...
    const result=await pool.query(
      `SELECT cpi.status, cpi.reference, cpi.total_amount, cpi.paid_at, cpi.expires_at, cpi.expires_at,
       ...
       WHERE cpi.reference=$1 AND cpi.app_id=$2
       LIMIT 1`,
      [reference,app.id]
    );
    const row=result.rows[0];
    if(!row) return sendJson(res,404,{error:"Intention de paiement introuvable.",code:"intent_not_found"});

    let status=String(row.status||"pending");
    if(status==="pending" && row.expires_at && new Date(row.expires_at).getTime()<Date.now()) status="expired";
    ...
    return sendJson(res,200,{
      ...
      plan:{id:row.plan_id,name:row.plan_name,amount:Number(row.plan_amount)},
      appId:row.app_id
    });
  }catch(error){
    console.error("[zakapro:checkout-status]",error);
    return sendJson(res,500,{error:"Impossible de vérifier le paiement.",code:"server"});
  }
}
