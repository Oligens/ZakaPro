/* ============================================================
   ...
   CONCURRENCE : verrou/version PostgreSQL par utilisateur
   ============================================================ */
...
const TABLES = {
  ...
  plans: { cols: ["id", "app_id", "name", "amount", "recurrence", "delivery", "created_at"], map: (r) => ({ id: r.id, appId: r.app_id, name: r.name, amount: Number(r.amount), recurrence: r.recurrence, delivery: r.delivery, createdAt: Number(r.created_at) }), unmap: (o) => [o.id, o.appId, o.name, o.amount, o.recurrence, o.delivery, o.createdAt] },
  ...
  deliveries: { cols: ["id", "at", "app_id", "app_name", "plan_name", "customer_phone", "address", "zone_name", "base_amount", "fee_amount", "total", "ref", "status", "delivered_at"], map: (r) => ({ id: r.id, at: Number(r.at), appId: r.app_id, appName: r.app_name, planName: r.plan_name, customerPhone: r.customer_phone, address: r.customer_phone ? r.address : r.address, zoneName: r.zone_name, baseAmount: Number(r.base_amount), feeAmount: Number(r.fee_amount), total: Number(r.total), ref: r.ref, status: r.status, deliveredAt: r.delivered_at ? Number(r.delivered_at) : undefined }), unmap: (o) => [o.id, o.at, o.appId, o.appName, o.planName, o.customerPhone, o.address, o.zoneName, o.baseAmount, o.feeAmount, o.total, o.ref, o.status, o.deliveredAt ?? null] },
...
async function checkSchema() {
  ...
  for (const [key, definition] of Object.entries(TABLES)) { const table = tableName(key); if (!available.has(table)) continue; const requiredColumns = ["user_id", ...definition.cols]; const columns = columnsByTable.get(table) || new Set(); for (const column of requiredColumns) if (!columns.has(column)) missing.push(`${table}.${column}`); }
  return missing;
}
...
async function getUserVersion(client, userId) {
  const result = await client.query(`INSERT INTO user_db_versions (user_id, version) VALUES ($1, 0) ON CONFLICT (user_id) DO NOTHING RETURNING version`, [userId]);
  if (result.rows[0]) return Number(result.rows[0].version);
  const current = await client.query(`SELECT version FROM user_db_versions WHERE user_id = $1`, [userId]);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (missingTables.length) return sendJson(res, 503, { error: "Schéma de base de données incomplet : exécutez les migrations Neon de ZakaPro.", code: "schema_missing", missingTables });
    ...
    if (req.method === "GET") {
      ...
      try {
        ...
        const version = await getUserVersion(client, userId);
        db.rev = version;
        for (const key of Object.keys(TABLES)) {
          const table = tableName(key); const order = key === "smsLog" || key === "engineLog" || key === "transactions" ? "ORDER BY at DESC LIMIT 200" : "";
          const { rows } = await client.query(`SELECT * FROM ${table} WHERE user_id = $1 ${order}`, [userId]); db[key] = rows.map(TABLES[key].map);
        }
        let settings = null;
        try { const settingsResult = await client.query("SELECT * FROM merchant_settings WHERE user_id = $1", [userId]); const s = settingsResult.rows[0]; if (s) settings = { alarmEnabled: s.alarm_enabled, volume: s.volume, monitoring: s.monitoring, urgency: s.urgency, webhookUrl: s.webhook_url || "", secret: s.secret }; } catch (err) { console.warn("[zakapro:db:settings]", err.message); }
        db.settings = settings;
        try { const counters = await client.query("SELECT COUNT(*)::int AS c FROM webhook_events WHERE user_id = $1", [userId]); db.webhookCount = counters.rows[0]?.c ?? 0; } catch (err) { console.warn("[zakapro:db:webhook]", err.message); }
    ...
    if (req.method === "POST") {
      ...
      try {
        await client.query("BEGIN");
        const lock = await client.query(`SELECT version FROM user_db_versions WHERE user_id = $1 FOR UPDATE`, [userId]);
        if (!lock.rows[0]) { await client.query(`INSERT INTO user_db_versions (user_id, version) VALUES ($1, 0)`, [userId]); }
        else { /* FOR UPDATE above holds the per-user row lock until COMMIT. */ }
        const currentVersionResult = await client.query(`SELECT version FROM user_db_versions WHERE user_id = $1`, [userId]);
        ...
        if (clientVersion !== currentVersion) {
          ...
          return sendJson(res, 409, { ok: false, code: "stale_write", error: "Cet état est obsolète : un autre onglet a déjà enregistré des modifications. Rechargez les données avant de sauvegarder.", serverRev: currentVersion });
        }
        ...
        await client.query(`UPDATE user_db_versions SET version = $2, updated_at = now() WHERE user_id = $1`, [userId, nextVersion]);
        await client.query("COMMIT");
    ...
    return sendJson(res, 500, { error: isTableMissing ? `Table base de données manquante : ${err.message}. Exécutez les migrations Neon de ZakaPro.` : "Erreur serveur — réessayez.", code: isTableMissing ? "missing_table" : "server", details: process.env.NODE_ENV === "development" ? err.message : undefined });
  }
}
